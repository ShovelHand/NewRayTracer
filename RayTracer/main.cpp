	...
	while (!pixelRows.empty())
	{
		...
		for (int row = range.first; row <= bottomRange; ++row) {
			...
			//	raysCast += 1;
				...
				if (r.dot(n) != 0) //the inequality is here until I can figure out why the plane appears in the sky.
				{
					...
					{	
						...
						image(row, col) = scene.artist.DrawGroundPixel(p, r);
					}
				}
					...
					if ((*itr)->CheckBoundsIntersection(eye, r, -10.0f, 10.0f))
					{
						...
						if (discriminant >= 0) //don't waste computation time if no intersection
						{
							...
							if (t < tmin)
							{
								...
								tmin = t;
								image(row, col) = scene.artist.DrawSpherePixel(*itr, p, r);
							}
						}
					}
...
/*Todo: get these metrics recorded again with the threaded scanning. 
...
//	ScanImage();
	...
	clock_t timeEnd = clock();
	cout << "Render time: " << float(timeEnd - timeStart) / 1000.0f << " seconds" << endl;
	image.save("../../RaycastImages/output.png"); ///< Does not work on Windows!
	image.show();
